...
function genkernel(NB, RM, RN, V, prefetch, K, L, boundary)
  ...
  local a,b,c = symmat("a",RM+2,1), symmat("b",K,1), symmat("c",RM,RN)
  ...
  local VimgP = &vector(double,V + RN - 1)

    for m = 0, RM+1 do
        loadA:insert(quote
            var [a[m][0]] : vector(double,V + RN - 1) = unalignedload(VimgP(&A[m*ldc])) -- it is not n*V
        end)
    end
  ...
  for  k=0, K-1 do
    loadkernel:insert(quote
      var [b[k][0]] : vector(double,V) = unalignedload(VP(&B[k*ldb]))
    end)
  end
